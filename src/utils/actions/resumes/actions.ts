...
export async function countResumes(type: 'base' | 'tailored' | 'all'): Promise<number> {
  ...
  const { count, error: countError, status, statusText } = await query;

  if (countError) {
    console.error('Supabase count error full details:', countError);
    console.error('Status:', status, 'Status Text:', statusText);
    return 0; // temporarily return 0 instead of throwing
  }

  console.log(`Total resumes (${type}) for user ${user.id}:`, count);
  return count || 0;
}
